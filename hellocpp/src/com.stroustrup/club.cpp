...
static void extract(const list<Club>& lc, list<Person*>& off)
...
static void extract_and_print(const list<Club>& lc)
{
	cout << "\nPrint officers:\n";
...
static bool Person_eq(const Person* x, const Person* y)
// equality through pointer
{
	return *x == *y;
}

static bool Person_lt(const Person* x, const Person* y)
// less than through pointer
{
	return *x < *y;
}

static void extract_and_print2(const list<Club>& lc)
{
...
static void make_person_list(list<Person*>& lst,
